...
class ClienteCreate(BaseModel):
    razao: str = Field(
        description="Nome do cliente ou Razão Social.", examples=["João LTDA"]
    )
    fantasia: Optional[str] = Field(
        default=None, description="Nome Fantasia.", examples=["Companhia LTDA"]
    )
    ...
    cnpj_cpf: str = Field(
        description="CPF ou CNPJ do cliente.", examples=["123.456.789-00"]
    )
    ...
    numero: str = Field(description="Número do endereço.", examples=[123])
    bairro: str = Field(description="Bairro.", examples=["Centro"])
    cidade: Optional[str] = Field(
        default="2033", description="ID da cidade no IXC.", examples=["2033"]
    )
    cep: str = Field(description="CEP do endereço.", examples=["12345-678"])
    telefone_celular: str = Field(
        description="Telefone de contato.", examples=["(12) 93456-7890"]
    )
    email: str = Field(description="Email de contato.")
    hotsite_email: Optional[str] = Field(
        default=None,
        description="Email do hotsite. Se não for fornecido, será o mesmo que o email principal.",
        examples=["example@example.com"],
    )
    tipo_assinante: str = Field(
        default="3", description="Tipo de assinante.", examples=["3"]
    )
    hotsite_acesso: Optional[str] = Field(
        default="1", description="Permite acesso ao hotsite.", examples=["1"]
    )
    iss_classificacao_padrao: Optional[str] = Field(
        default="99", description="Indicador de Receita.", examples=["99"]
    )
    contribuinte_icms: Optional[str] = Field(
        default="N", description="Contribuinte do ICMS.", examples=["N"]
    )
    tipo_localidade: str = Field(
        default="U", description="Tipo de localidade.", examples=["U"]
    )
    cob_envia_email: str = Field(
        default="S", description="Enviar notificações por e-mail.", examples=["S"]
    )
    cob_envia_sms: str = Field(
        default="S", description="Enviar notificações por SMS.", examples=["S"]
    )
    senha: str = Field(
        description="Senha do cliente no hotsite.", examples=["12345678"]
    )
    id_tipo_cliente: str = Field(
        default="1", description="ID do tipo de cliente.", examples=["1"]
    )
    ...
    crm_data_novo: str = Field(
        default_factory=lambda: date.today().isoformat(),
        description="Data em que o cliente foi marcado como novo no CRM.",
        examples=["2023-01-01"],
    )

    @model_validator(mode="after")
    def set_hotsite_email(self) -> "ClienteCreate":
        if not self.hotsite_email:
            self.hotsite_email = self.email
        return self
