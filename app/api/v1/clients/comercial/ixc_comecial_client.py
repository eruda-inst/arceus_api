...
class ComercialIXCCliente(ixc_client.IXCCliente):
    @classmethod
    async def get_status_acesso(cls, id_contrato: int) -> Any:
    ...
    @classmethod
    async def post_leads(cls, lead: schemas.LeadIn) -> Any:
    ...
    @classmethod
    async def get_login(cls, id_cliente: PositiveInt) -> Any:
    ...
    @classmethod
    async def get_lead_by_cpf_cnpj(cls, cnpj_cpf: str) -> Any:
        grid_param = [{"TB": "contato.cnpj_cpf", "OP": "=", "P": cnpj_cpf}]
        payload = {
            "grid_param": json.dumps(obj=grid_param),
        }
        data = await cls._make_request(endpoint="contato", payload=payload)
        return data

    @classmethod
    async def put_lead(cls, lead_id: int, lead: Dict[str, Any]) -> Any:
        """
        Este campo "data_cadastro" é obrigatório na API do IXC (temos que mandar alguma coisa), porém o que é mandado é descartado e a data é gerada automaticamente pela própria API deles.
